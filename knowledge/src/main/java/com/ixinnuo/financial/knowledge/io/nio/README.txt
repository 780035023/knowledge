...
CLIENT EXECUTION【客户端执行，使用浏览器即可】
================
...
SOURCE CODE OVERVIEW【源码概述】
====================

The main class is Server, which handles program startup, and is
subclassed by the "B*" and "N*" server classes.【启动类为Server，B*和N*为其子类 】
...
【不论哪种模式，只要建立了连接，就创建一个ChannelIO对象处理所有输入输出。非安全模式下，SocketChannel相应
方法直接调用。然而，安全模式下，首先需要更多操作把channel转成安全的，加解密数据，最终正确 关闭消息】
